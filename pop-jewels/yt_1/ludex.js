...
default", AdService: "platform", PlacementId: "rewarded_video_platform", SecondsFirstTime: 0, SecondsBetweenAds: 0 }],
      ...
      Enabled: false }, RemoteConfig: { Enabled: false } } }, Analytics: { Enabled: false, Distribution: {
    ...
    "extensions/ludex/libs/init-core.ts"() {
      ...
      initGoogleAnalytics = /* @__PURE__ */ __name((userId, entryPoint, tagIds) => {
        try {
          ...
          "";
          const resolvedBuild = `${buildVersion}${distSuffix}`;
      ...
      finalizeAnalyticsInitialization = /* @__PURE__ */ __name(() => {
        ...
        try {
          logAnalyticsEvent(AnalyticsEvents.APP_LAUNCH);
          codex.event.catchUp(Events10.GAME_SDK_READY, () => {
            logAnalyticsEvent(AnalyticsEvents.APP_INITIALIZED);
          });
          codex.event.catchUp(Events10.PLAYER_INFO_LOADED, updateUserProperties);
        } catch (error) {
          logError(error);
        } finally {
          codex.event.emit(Events10.ANALYTICS_READY);
        }
      }, "finalizeAnalyticsInitialization");
    ...
    "extensions/ludex/src/game-sdk/sdk/Context.ts"() {
      ...
      _Context = class _Context {
        ...
        initContextInfo(contextId2, contextType2) {
          const { Array: A9, String: S9 } = Ludex.Utils;
          this.contextIdsByCreate$ = {};
          this.playerIdsInContexts$ = {};
          this.contextIdsByChoose$ = A9.fillWith(10, () => S9.generateObjectId());
        ...
        createPlayerIdsInContext$(contextId2, maxPlayers) {
          if (!this.playerIdsInContexts$[contextId2]) {
            ...
            const playerIds = A9.fillWith(rand, () => S9.generateObjectId());
    ...
    "extensions/ludex/src/game-sdk/sdk/Extra.ts"() {
      ...
      _Extra = class _Extra {
        ...
        async checkAndSubscribeBotAsync() {
          ...
          const {
            Plugins: { Analytics: { Events: Events10 } = {} }
          } = Ludex;
          Events10 && analytics.event(Events10.BOT_SUBSCRIBE_POPUP);
          try {
            await this.waitSDKInitiatedAsync();
            await this.adapter$.player.subscribeBotAsync();
            Events10 && analytics.event(Events10.BOT_SUBSCRIBE, { success: true });
            return true;
          } catch (error) {
            console.warn(error);
            Events10 && analytics.event(Events10.BOT_SUBSCRIBE, { success: false });
            return false;
          }
        }
        async checkAndCreateShortcutAsync() {
          ...
          Events10 && analytics.event(Events10.SHORTCUT_CREATE_POPUP);
          try {
            await this.waitSDKInitiatedAsync();
            await this.adapter$.createShortcutAsync();
            Events10 && analytics.event(Events10.SHORTCUT_CREATE, { success: true });
          } catch (error) {
            console.warn(error);
            Events10 && analytics.event(Events10.SHORTCUT_CREATE, { success: false });
          }
        }
...
tion"];
      ...
      _Tournament = class _Tournament {
        ...
        createDefaultTournaments$(length) {
          const {
            Utils: { String: S9, Array: A9, Number: N },
            Configs: {
              Mockup: {
                GameSDK: {
          ...
          return A9.length(length).map(async () => {
        ...
        createGlobalTournaments$(length) {
          ...
          return A9.length(length).map(async () => {
            ...
            const leaders = A9.length(leaderLength).map(() => {
              return {
...
amentType")
        ...
        getTournamentLeader$(tournamentId, playerId2) {
          ...
          return A9.search(tournamentLeaders, (leader) => leader.playerId === playerId2) ?? null;
        }
        ...
        writeTournamentLeader$(tournamentId, leader) {
          ...
          const tournamentLeaders = this.getTournamentLeadersData$(tournamentId);
          const leaderIndex = A9.searchIndex(
            tournamentLeaders,
    ...
    "extensions/ludex/src/game-sdk/sdk/GameSDK.ts"() {
      ...
      _GameSDK = class _GameSDK {
        ...
        async startGameAsync(withEvent = true) {
          ...
          if (withEvent) {
            codex.event.emit(Events10.GAME_SDK_STARTED);
          }
        }
        setLoadingProgress(percentage) {
        ...
        async getTournamentAsync() {
          ...
          const { Array: A9, Valid: V36 } = Ludex.Utils;
          ...
          const wantedTournament = A9.search(tournaments, (tournament) => {
...
EDED"];
      ...
      _YTPlayer = class _YTPlayer extends Player_default {
        ...
        async getDataAsync(keys) {
          ...
          try {
            ...
            const {
              Utils: { Json: J6, Valid: V36, Object: O32 }
            } = Ludex;
    ...
    "extensions/ludex/src/game-sdk/adapters/youtube-playables/YTAdapter.ts"() {
      ...
      _YTAdapter = class _YTAdapter extends GameSDK_default2 {
        ...
        setupLifecycleListeners$() {
          const { Events: Events10 } = Ludex;
          const { event } = codex;
          this.extra.onPause(() => {
            event.emit(Events10.VISIBILITY_HIDDEN);
          });
          this.extra.onResume(() => {
            event.emit(Events10.VISIBILITY_VISIBLE);
          });
        }
  ...
  var require_eventemitter3 = __commonJS({
    "node_modules/eventemitter3/index.js"(exports, module) {
      ...
      __name(Events10, "Events");
      if (Object.create) {
        Events10.prototype = /* @__PURE__ */ Object.create(null);
        if (!new Events10().__proto__) prefix = false;
      }
      ...
      function clearEvent(emitter, evt) {
        if (--emitter._eventsCount === 0) emitter._events = new Events10();
        else delete emitter._events[evt];
      }
      __name(clearEvent, "clearEvent");
      function EventEmitter2() {
        this._events = new Events10();
        this._eventsCount = 0;
      }
...
n") {
  ...
  window.CommitId = "561b4e15-ee527683";
  ...
  var {
    Configs,
    Utils: { Array: A3, Valid: V5, Browser: B }
  } = Ludex;
  var _Context2 = class _Context2 {
    ...
    isValidEntryPointData$(entryPointData2) {
      return V5.isObject(entryPointData2);
    }
  ...
  var createEntryPointActionStep = /* @__PURE__ */ __name(() => ({
    ...
    runAsync: /* @__PURE__ */ __name(async (context) => {
      try {
        ...
        if (!V6.isString(action)) return true;
  ...
  var createContinueTournamentStep = /* @__PURE__ */ __name(() => ({
    ...
    runAsync: /* @__PURE__ */ __name(async (context) => {
      try {
        const { match, player } = codex;
        const playerId2 = player.getPlayerId();
        if (V7.isEmpty(context.contextId)) {
...
tarell,"Fira Sans","Droid Sans","Helvetica Neue",sans-serif`;
  ...
  var {
    ...
    Utils: { Object: O8, String: S2, Valid: V11 }
  } = Ludex;
  var _LeaderboardAPI = class _LeaderboardAPI {
    ...
    async getLeadersAsync(id, url, payload) {
      const params = S2.params(payload);
      let result3;
    ...
    async getLeaderboardsAsync(options) {
      let result3;
      const params = S2.params(options);
    ...
    async createLeaderboardAsync(payload) {
      const id = payload._id ?? S2.generateObjectId();
...
customData"]);
  ...
  var _LogMatchStartHandler = class _LogMatchStartHandler extends BaseHandler_default {
    async processAsync(matchState) {
      ...
      if (Events4) {
        analytics.event(Events4.MATCH_START, {
          level: level2,
          game_mode: S3.toUpperCamelCase(mode),
    ...
    getLevelName$(level2) {
      return `Level ${S3.padStart(level2.toString(), 5, "0")}`;
    }
  };
...
i n\xE0o!`
  ...
  var _ProfileAPI = class _ProfileAPI {
    ...
    async getPlayersAsync(payload) {
      const params = S6.params(payload);
      let result3;
  ...
  var _NormalAdInstance = class _NormalAdInstance extends AdInstance3 {
    ...
    constructor(type, placementId) {
      super(type, placementId);
      this.service$ = S7.toSnakeCase(GameSDK.getSDKName());
      this.instance$ = null;
    }
  ...
  var _MagicCodex = class _MagicCodex extends codex_default {
    ...
    async boot() {
      ...
      this.event.emit(Events8.CORE_BOOTING);
      ...
      if (!V35.isDebugger()) {
        this.event.on(Events8.MODULE_PLUGIN_READY, this.handleProtectGameInstance$);
      }
      ...
      this.event.catchUp(Events8.GAME_SDK_READY, this.addDevPlugins$);
      console.groupEnd();
    }
    async validateCoreFlow$() {
      if (false) return;
      await T2.sleepAsync(5e3);
      const eventInfo = this.event.getEventListeners(Events8.CORE_BOOTING);
    ...
    async start() {
      ...
      this.event.emit(Events8.CORE_STARTING);
      ...
      if (GameSDK.isGameSDK("FacebookInstant")) {
        this.event.catchUp(Events8.GAME_SDK_STARTED, this.processContextSession$);
      } else {
      ...
      this.event.emit(Events8.CORE_READY);
    ...
    addPlugins$() {
      ...
      this.installPlugin$("Analytics", empty_script_default);
    ...
    updateMonitorErrorUserAndMetadata$ = /* @__PURE__ */ __name(() => {
      this.event.catchUp(Events8.PLAYER_INFO_LOADED, this.processWhenPlayerInfoLoaded$);
  ...
  var initGame = /* @__PURE__ */ __name(async () => {
    ...
    codex2.event.catchUp(Events9.REQUEST_CORE_START, startGame);
  }, "initGame");
  var isGameStarted = false;
  var startGame = /* @__PURE__ */ __name(() => {
    if (isGameStarted) return;
    if (codex.event.getEventEmitCount(Events9.GAME_SDK_READY) < 1) {
      codex.event.catchUp(Events9.GAME_SDK_READY, startGame);
      return;
    }
    codex.start();
    isGameStarted = true;
