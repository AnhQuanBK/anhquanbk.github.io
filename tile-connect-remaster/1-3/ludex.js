      ...
      async getAllAddOnsAsync$() {
        try {
          ...
          if (this.devPlaygroundAppId$ && V36.isDebugger()) {
            payload = {
              productId: this.devPlaygroundAppId$
            };
          }
          const response = await this.sdk$.iap.getAllAddOnsAsync(payload);
          return response;
        } catch (error) {
  ...
  "extensions/ludex/src/game-sdk/adapters/msgames/MSPlayer.ts"() {
    ...
    _MSPlayer = class _MSPlayer extends Player_default {
      ...
      async getDataAsync(keys) {
        ...
        let gameId;
        if (this.devPlaygroundAppId$ && V36.isDebugger()) {
          gameId = this.devPlaygroundAppId$;
        }
        const data = await this.requestCloudSaveDataAsync$(gameId);
        const requiredData = {};
        for (const key of keys) {
          if (O32.hasOwn(data, key)) {
      ...
      async setDataAsync(data) {
        ...
        await this.updateCloudSaveDataAsync$(data, gameId);
      }
      async requestCloudSaveDataAsync$(gameId) {
        try {
          let response = {};
          if (gameId) {
            response = await this.adapter$.sdk.cloudSave.getDataAsync({ gameId });
          } else {
            response = await this.adapter$.sdk.cloudSave.getDataAsync();
          }
          if ("code" in response) {
      ...
      async updateCloudSaveDataAsync$(data, gameId) {
        try {
          ...
          const response = await this.adapter$.sdk.cloudSave.saveDataAsync(config);
          if ("code" in response) {
            const errorCode = response.code;
            throw new Error(errorCode);
          }
...
window.CommitId = "f74f5c41-50e08641";
window.BuildVersion = 5;
